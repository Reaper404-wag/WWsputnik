...
def fetch_tle_data_oneweb(group: str = "oneweb"):
...
def tle_epoch(epoch_str):
    epoch = float(epoch_str)
    year = 2000 + int(epoch // 1000)
    day = int(epoch % 1000)
    fraction = epoch - int(epoch)
    dt = datetime(year, 1, 1) + timedelta(days=day - 1, seconds=fraction * 86400)
    return dt


def decode_tle(tle_list):
    try:
        line1 = tle_list[0].replace(' ', '').replace('\n', '')
        line2 = tle_list[1]
        line3 = tle_list[2]
        ...
        get_tle = tle_epoch(mylist[3][:-7]) #Эпоха, дату и время получения указанных TLE-строк
